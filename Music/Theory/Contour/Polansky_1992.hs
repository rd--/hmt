...
import qualified Music.Theory.Permutations.List as Permutations.List {- hmt -}
...
>>> contour_description_ix (contour_description "abdc") (0,3)
LT
-}
...
>>> let c = ["abc","bbb","cba"]
>>> map (uniform.contour_description) c
[True,True,True]
-}
uniform :: Contour_Description -> Bool
uniform (Contour_Description _ m) = List.all_equal (M.elems m)

{- | 'True' if contour does not containt any 'EQ' elements.

>>> let c = ["abc","bbb","cba"]
>>> map (no_equalities.contour_description) c
[True,False,True]
-}
...
all_contours n =
  let n' = contour_description_lm n
      ix = all_indices n
      cs = filter (not . null) (Set.List.powerset [LT, EQ, GT])
      pf = concatMap Permutations.List.multiset_permutations . Set.List.expand_set n'
...
>>> map implication [(LT,EQ),(EQ,EQ),(EQ,GT)]
[Just LT,Just EQ,Just GT]
-}
...
>>> map (length.possible_contours) [3,4,5]
[13,75,541]
-}
...
>>> map (length.impossible_contours) [3,4,5]
[14,654,58508]
-}
...
>>> map contour_description_lm [2..7]
[1,3,6,10,15,21]

>>> map (\n -> 3 ^ n) (map contour_description_lm [2..6])
[3,27,729,59049,14348907]
-}
...
>>> contour_truncate (contour_description [3,2,4,1]) 3 == contour_description [3,2,4]
True
-}
...
>>> let d = contour_description [3,2,4]
>>> d `contour_is_prefix_of` c
True
-}
...
>>> let d = contour_description [3,2,4,1]
>>> map (contour_eq_at c d) [0..4]
...
{- | Derive an 'Integral' contour that would be described by 'Contour_Description'.
Diverges for impossible contours.

>>> draw_contour (contour_description "abdc")
[0,1,3,2]
-}
...
draw_contour d =
  let n = contour_description_n d
      ...
      step (i, j) ns =
        let c = contour_description_ix d (i, j)
            i' = ns !! i
            j' = ns !! j
        ...
        in if c == c'
            then Nothing
              ...
              let j'' = case c of
                    ...
                    EQ -> i'
                    GT -> i' - adjustment j'
              in Just (List.replace_at ns j j'')
      refine [] ns = ns
...
>>> draw_contour (contour_description_invert c)
[3,2,0,1]
-}
contour_description_invert :: Contour_Description -> Contour_Description
contour_description_invert (Contour_Description n m) =
  Contour_Description n (M.map Ord.ord_invert m)

-- * Construction

{- | Function to perhaps generate an element and a new state from an initial state.
This is the function provided to 'unfoldr'.
-}
...
to generate possible elements, a 'Conforms_f' that the result sequence
must conform to at each step, an 'Int' to specify the maximum number
of elements to generate when searching for a solution, and an initial
state.

>>> let b_f i = Just (i,i+1)
>>> let c_f i x = odd (sum x `div` i)
>>> build_sequence 6 b_f c_f 20 0
(Just [1,2,6,11,15,19],20)
-}
build_sequence ::
  Int ->
  Build_f st e ->
...
>>> let c = contour_description "atdez"
>>> let st = mkStdGen 2347
>>> fst (build_contour f c 64 st) -- ?
Nothing
...
build_contour f c z =
  let n = contour_description_n c
      g i r =
        let d = contour_description r -- traceShow r
...
>>> import System.Random
>>> let f = Just . randomR ('a','z')
>>> let c = contour_description "atdezjh"
>>> let st = mkStdGen 2347
>>> fst (build_contour_retry f c 64 8 st)
Just "bxetzwv"

>>> c
000000 22022 0000 000 22 2
-}
build_contour_retry ::
  (Ord e) =>
  Build_f st e ->
...
>>> let c = contour_description "atdezjh"
>>> let st = mkStdGen 2347
>>> length (build_contour_set f c 64 64 st)
550
-}
build_contour_set ::
  (Ord e) =>
...
>>> import System.Random
>>> let f = Just . randomR ('a','f')
>>> let c = contour_description "cafe"
>>> let st = mkStdGen 2346836
>>> let r = build_contour_set_nodup f c 64 64 st
>>> filter ("c" `isPrefixOf`) r
["cafd","cbfe"]
...
>>> List.compare_adjacent ex_1
[GT,GT,LT,LT]

>>> show (contour_half_matrix ex_1)
"2221 220 00 0"

>>> draw_contour (contour_description ex_1)
[3,2,0,1,3]

>>> let d = contour_description_invert (contour_description ex_1)
>>> (show d,is_possible d)
("0001 002 22 2",True)
-}
ex_1 :: [Rational]
ex_1 = [2, 3 % 2, 1 % 2, 1, 2]
...
>>> show (contour_description ex_3)
"22222 2101 000 01 2"

>>> contour_description_ix (contour_description ex_3) (0,5)
GT

>>> is_possible (contour_description ex_3)
True
-}
ex_3 :: [Integer]
ex_3 = [12, 7, 6, 7, 8, 7]
