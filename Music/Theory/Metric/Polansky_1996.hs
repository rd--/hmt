...
d_dx_abs f = map abs . List.d_dx_by f

{- | Ordered linear magnitude (no delta), p.300

>>> olm_no_delta' [0,2,4,1,0] [2,3,0,4,1]
1.25
-}
...
>>> olm_general (abs_of (-)) [0,2,4,1,0] [2,3,0,4,1]
1.25

>>> olm_general (abs_of (-)) [1,5,12,2,9,6] [7,6,4,9,8,1]
4.6
-}
...
>>> map (sqr_abs_ix_dif (-) [0,2,4,1,0]) [0..3]
[4,4,9,1]

>>> map (sqr_abs_ix_dif (-) [2,3,0,4,1]) [0..3]
[1,9,16,9]
-}
...
>>> olm (abs_of dif_r) (abs_ix_dif dif_r) (const 1) [1,5,12,2,9,6] [7,6,4,9,8,1]
4.6
...
>>> map second_order_binonial_coefficient [2..10]
...
{- | 'List.d_dx_by' of 'flip' 'compare'.

>>> direction_interval [5,9,3,2]
[LT,GT,GT]

>>> direction_interval [2,5,6,6]
[LT,LT,EQ]
-}
direction_interval :: Ord i => [i] -> [Ordering]
direction_interval = List.d_dx_by (flip compare)
...
>>> direction_interval [5,3,6,1,4]
[GT,LT,GT,LT]

>>> direction_interval [3,6,1,4,2]
[LT,GT,LT,GT]
...
>>> combinatorial_magnitude_matrix (abs_of (-)) [5,3,2,6,9]
[[2,3,1,4],[1,3,6],[4,7],[3]]
-}
combinatorial_magnitude_matrix :: Interval a n -> [a] -> [[n]]
combinatorial_magnitude_matrix = Contour.half_matrix_f
...
>>> ocm (abs_of (-)) [1,6,2,5,11] [3,15,13,2,9]
5.2

>>> ocm (abs_of (-)) [1,5,12,2,9,6] [7,6,4,9,8,1]
3.6
-}
...
>>> ocm_absolute_scaled (abs_of (-)) [1,6,2,5,11] [3,15,13,2,9]
0.4
