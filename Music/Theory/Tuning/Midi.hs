...
>>> let f = d12_midi_tuning_f (Tuning.tn_equal_temperament 12,0,0)
...
-- | Variant of `cps_midi_tuning_f` that loads the tuning from a named Scala tuning file.
scala_cps_midi_tuning_f :: (String, Double, Pitch.Midi, Int) -> IO Sparse_Midi_Tuning_f
scala_cps_midi_tuning_f (nm, f0, k, g) = do
  t <- Scala.scl_load_tuning nm
  return (cps_midi_tuning_f (t, f0, k, g))
...
> let f = cps_midi_tuning_f (Tuning.tn_equal_temperament 12,midi_to_cps 0,0,127)
...
>>> import Music.Theory.Array.Text
>>> t <- scala_cps_midi_tuning_f ("bohlen-p",Tuning.midi_to_cps 0,0,128)
>>> let tbl = gen_fmnn_tuning_tbl t
>>> putStr $ unlines $ mnn_fmnn_table_pp (take 12 tbl)
Midi Note Frequency
--------- ---------
        0    0.0000
        1    1.3324
