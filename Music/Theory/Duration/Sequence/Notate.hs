...
m_divisions_rq z =
  fmap
    ( rqt_tuplet_subdivide_seq_sanity_ (1 / 16)
        . map rqt_set_to_cmn
    )
    . rqt_separate z
...
zip_hold lhs_f rhs_f =
  let f r x t =
        case (x, t) of
          ...
          (x0 : x', t0 : t') ->
            let x'' = if rhs_f t0 then x else x'
