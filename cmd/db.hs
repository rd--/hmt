...
db_load_ty ty =
    case ty of
      "plain" -> Plain.db_load_utf8
      "json" -> JSON.db_load_utf8
      "csv" -> CSV.db_load_utf8
      _ -> error "db_load_ty"

db_store_ty :: String -> FilePath -> Common.DB String String -> IO ()
db_store_ty ty =
    case ty of
      "plain" -> Plain.db_store_utf8
      "json" -> JSON.db_store_utf8
      "csv" -> CSV.db_store_utf8
      _ -> error "db_store_ty"
...
main = do
  a <- getArgs
  case a of
    ["convert",input_ty,output_ty,input_fn,output_fn] -> convert (input_ty,output_ty) (input_fn,output_fn)
